...
function compareMetadata(metadata, submission) {
  assertEqual("track_id", TRACK_ID, submission.track_id); assertEqual("benchmark", metadata.benchmark, "ecadd-challenge-test"); assertEqual("score_model", metadata.scoreModel, submission.score_model); assertEqual("artifact_binary_sha256", metadata.artifactSha256, submission.artifact_binary_sha256); assertEqual("architecture_diagram_path", metadata.architectureDiagram?.path, submission.architecture_diagram_path); assertEqual("architecture_diagram_sha256", metadata.architectureDiagram?.sha256, submission.architecture_diagram_sha256); assertEqual("architecture_diagram_size_bytes", metadata.architectureDiagram?.bytes, submission.architecture_diagram_size_bytes); assertEqual("score", metadata.localScore, submission.metrics?.score); assertEqual("metrics.logical_qubits", metadata.metrics?.qubits, submission.metrics?.logical_qubits); assertEqual("metrics.toffoli_count", metadata.metrics?.toffoli, submission.metrics?.toffoli_count); assertEqual("metrics.artifact_binary_size_bytes", metadata.artifactBytes, submission.metrics?.artifact_binary_size_bytes);
}
