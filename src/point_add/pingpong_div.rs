61, 61, 61, 61, 60, 60, 60, 60, 60, 60, 59, 59, 58, 58, 58, 56, 56, 55, 55, 55, 53, 53, 53, 53, 52, 52, 52, 52, 52, 51, 51, 50, 50, 50, 49, 49, 48, 48, 46, 46, 46, 46, 46, 46, 46, 46, 46, 45, 45, 45, 45, 44, 44, 43, 42, 42, 42, 42, 41, 41, 40, 39, 38, 38, 37, 37, 36, 36, 36, 36, 35, 34, 34, 34, 34, 33, 32, 32, 32, 32, 32, 31, 31, 30, 30, 30, 29, 29, 28, 27, 27, 27, 26, 25, 24, 24, 24, 23, 22, 22, 22, 22, 22, 22, 21, 21, 20, 20, 20, 18, 18, 18, 18, 18, 18, 18, 17, 17, 16, 15, 14, 13, 12, 11, 10, 10, 10, 10, 10, 10, 10, 10, 10, 10];
...
fn conditional_mod_negate(b: &mut B, control: QubitId, value: &[QubitId]) {
    ...
    let f = U256::MAX
        .wrapping_sub(SECP256K1_P)
        .wrapping_add(U256::from(1));
    let acc = replay_fold_target(value);
    let c = f.wrapping_sub(U256::from(1));
    // Under the fold peak budget, park the top of the borrow ladder (the
    // positions above the constant's top bit) so the endpoint negate does not
    // become the binding ladder at the terminal state; ~1.5 T per parked
    // borrow, a handful of calls per shot.
    let window = endpoint_fold_window();
    let hi = highest_set_bit(c);
    let last = (acc.len() - 2).min(hi.saturating_add(window));
    let ladder = last + 1;
    let room = fold_peak_budget().map_or(usize::MAX, |budget| budget.saturating_sub(b.active_qubits) as usize);
    let park_low = if ladder > room { (ladder - room).min(last.saturating_sub(hi)) } else { 0 };
    if park_low > 0 && std::env::var_os("SUB4_PP_FOLD_BANKS_OFF").is_none() {
        cconst_nbit_direct_trunc_fast_parked(b, acc, c, control, window, park_low, false);
        return;
    }
    csub_nbit_const_direct_trunc_fast_dead_low(b, acc, c, control, window, false);
}
...
/// Peak budget for the fold ladder (`SUB4_PP_FOLD_PEAK`, default `SUB4_PP_PEAK`).
fn fold_peak_budget() -> Option<u32> {
    static SLOT: std::sync::OnceLock<Option<u32>> = std::sync::OnceLock::new();
    *SLOT.get_or_init(|| {
        std::env::var("SUB4_PP_FOLD_PEAK")
            .ok()
            .or_else(|| std::env::var("SUB4_PP_PEAK").ok())
            .and_then(|v| v.parse().ok())
    })
}

/// Number of banks the fold ladder is split into so that its live carry wires
/// (one bank's ladder plus the retained boundary carries) fit under the fold
/// peak budget at the current live count. 1 = the unbanked ladder.
fn fold_banks_for(b: &B, num_carries: usize) -> usize {
    if std::env::var_os("SUB4_PP_FOLD_BANKS_OFF").is_some() {
        return 1;
    }
    let Some(budget) = fold_peak_budget() else { return 1 };
    let room = budget.saturating_sub(b.active_qubits) as usize;
    if num_carries <= room {
        return 1;
    }
    for banks in 2..=num_carries {
        if num_carries.div_ceil(banks) + (banks - 1) <= room {
            return banks;
        }
    }
    num_carries.max(1)
...
/// One leading bank of the fold ladder over ladder positions `lo..hi`
/// (absolute indices into `acc`, `1 <= lo < hi <= width - 2`). Adds the
/// selected operand bits at those positions with carry-in `carry_in` and
/// returns the carry out of position `hi - 1` in a fresh wire that stays
/// live. Interior carries are measurement-uncomputed as in
/// [`fused_fold_maskfree`].
fn fused_fold_bank(
    b: &mut B,
    acc: &[QubitId],
    controls: &dyn Fn(usize) -> Vec<QubitId>,
    carry_in: QubitId,
    lo: usize,
    hi: usize,
) -> QubitId {
    ...
    for offset in 0..n {
        let i = lo + offset;
        let previous = if offset == 0 { carry_in } else { carries[offset - 1] };
        let selectors = controls(i);
        if selectors.is_empty() {
            b.cx(previous, acc[i]);
            b.ccx(previous, acc[i], carries[offset]);
    ...
    // Reverse: the top carry is kept (it is the true carry-out); the rest are
    // measured away exactly as in the unbanked ladder.
    ...
    let out = carries[n - 1];
    b.free_vec(&carries[..n - 1]);
    out
}

/// Phase-conditioned erase of a retained bank carry `carry`: measure it away
/// and, under the outcome, recompute the bank's carry-out from its post-add
/// bits (`acc_new < operand + carry_in`, i.e. the carry of
/// `!acc_new + operand + carry_in`), phase-flip on it, and uncompute the chain
/// again leaving `acc` unchanged.
fn fused_fold_bank_erase(
    b: &mut B,
    acc: &[QubitId],
    controls: &dyn Fn(usize) -> Vec<QubitId>,
    carry_in: QubitId,
    ...
    for offset in (0..n).rev() {
        ...
        if selectors.is_empty() {
            b.cx(previous, carries[offset]);
            let m = b.alloc_bit();
            b.hmr(carries[offset], m);
            b.cz_if(previous, acc[i], m);
            b.cx(previous, acc[i]);
        } else {
            let operand = selectors[0];
            for &control in &selectors[1..] {
...
/// [`fused_fold_maskfree`] with its carry ladder split into banks so that at
/// most one bank's carries (plus the retained boundary carries) are live at
/// once. Byte-identical to the unbanked fold whenever one bank fits.
fn fused_fold_banked(
    b: &mut B,
    acc: &[QubitId],
    f: U256,
    negative_f: &[bool],
    ...
    let width = acc.len();
    let num_carries = width.saturating_sub(3);
    let banks = if width >= 8 { fold_banks_for(b, num_carries) } else { 1 };
    if banks <= 1 {
        return fused_fold_maskfree(b, acc, f, negative_f, plus_f, plus_2f, minus_f, first_carry);
    }
    let controls = |index: usize| fused_operand_controls(f, negative_f, index, plus_f, plus_2f, minus_f);
    // Position 0 has no carry-in; the ladder positions are 1..=num_carries
    // and the terminal stage closes width-2 and width-1. The leading banks
    // take a prefix of the ladder positions; the final bank is the ordinary
    // fold over the rest, seeded with the retained carry.
    for control in controls(0) {
        b.cx(control, acc[0]);
    }
    let per_bank = num_carries.div_ceil(banks);
    let mut boundaries: Vec<(usize, usize, QubitId)> = Vec::new();
    let mut carry_in = first_carry;
    let mut lo = 1;
    for _ in 0..banks - 1 {
        let hi = (lo + per_bank).min(1 + num_carries - 1);
        if hi <= lo {
            break;
        }
        let out = fused_fold_bank(b, acc, &controls, carry_in, lo, hi);
        boundaries.push((lo, hi, out));
        carry_in = out;
        lo = hi;
    }
    // Final bank over acc[lo-1..]: its position 0 (= absolute lo-1) already
    // holds its sum, so its operand is suppressed; the ladder resumes at
    // absolute position lo with `carry_in`.
    let shift = lo - 1;
    let tail_controls = move |index: usize| if index == 0 { Vec::new() } else { controls(index + shift) };
    fused_fold_tail(b, &acc[shift..], &tail_controls, carry_in);
    for k in (0..boundaries.len()).rev() {
        let (blo, bhi, carry) = boundaries[k];
        let cin = if k == 0 { first_carry } else { boundaries[k - 1].2 };
        fused_fold_bank_erase(b, acc, &controls, cin, blo, bhi, carry);
    }
}

/// The unbanked fold body of [`fused_fold_maskfree`] with the operand table
/// abstracted so a bank can run it over a suffix of the window.
fn fused_fold_tail(b: &mut B, acc: &[QubitId], controls: &dyn Fn(usize) -> Vec<QubitId>, first_carry: QubitId) {
    let width = acc.len();
    for control in controls(0) {
        b.cx(control, acc[0]);
    }
    ...
    for offset in (0..num_carries).rev() {
        let i = lo + offset;
        let previous = if offset == 0 { first_carry } else { carries[offset - 1] };
        let selectors = controls(i);
        if selectors.is_empty() {
            b.cx(previous, carries[offset]);
            let measured = b.alloc_bit();
