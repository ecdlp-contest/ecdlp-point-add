...
pub fn build() -> Vec<Op> {
    ...
    // CALIBRATED RISK BUDGET (2026-09-12, see SUBMISSION.md): every finite
    // window below was calibrated on the trusted evaluator at small widths
    // (where failures are countable) and extrapolated on the measured 2^-w
    // scaling; the walk round count was set from the exact classical model's
    // convergence tail over 1.95M fresh-draw shots. Combined per-draw failure
    // expectation ~2.3e-2 (rounds 1.8e-2, fold 1.3e-3, flag 3e-4, chunk 3.2e-3).
    set_default_env("SUB4_PP_ROUNDS", "750");
    ...
    set_default_env("SUB4_PP_PEAK", "1331");
    // Banked fold / parked endpoint ladders are available (see pingpong_div.rs)
    // but disabled for this stream: a very large fold budget keeps the ladders
    // unbanked, byte-identical to the accepted construction.
    set_default_env("SUB4_PP_FOLD_PEAK", "100000");
    set_default_env("SUB4_PP_WALK_PEAK", "1331");
    set_default_env("SUB4_PP_REPLAY_CHUNK", "96");
    set_default_env("SUB4_PP_REPLAY_CHUNK_COMPARE", "33");
    set_default_env("SUB4_PP_REPLAY_FOLD_WINDOW", "68");
    set_default_env("SUB4_PP_REPLAY_FOLD_WINDOW_MUL", "68");
    ...
    set_default_env("SUB4_PP_REPLAY_FLAG_COMPARE", "36");
