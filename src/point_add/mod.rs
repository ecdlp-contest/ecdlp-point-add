...
//! Reversible secp256k1 ECDLP point-add circuit runtime.
//!
//! The active two-block affine schedule and binary-GCD replay bindings are
//! generated from `profiles/ecdlp-binary-gcd-generated-v1.json`. The named
//! approximate bad event remains `x(P+Q)=Qx`, equivalently `P=-2Q`.
//!
//! Formal semantics, lowering evidence, operation-stream parity, and trusted
//! evaluation are separate authority layers.
...
mod binary_gcd_in_place_inactive;
...
/// Canonical modular negation on inputs in `[0, p)`.
///
/// `mod_neg_inplace_fast` deliberately implements the reversible integer map
/// `v -> p-v`, so it sends zero to the non-canonical word `p`.  Point-add's
/// two-block binary-GCD schedule needs `-lambda` as a canonical field element.
/// Capture the zero predicate, apply the fast negation, map the exceptional
/// output `p` back to zero, then recompute the predicate from that canonical
/// output to clear the flag.
fn mod_neg_inplace_canonical_fast(b: &mut B, v: &[QubitId], p: U256) {
    let was_zero = b.alloc_qubit();
    cmp_eq_zero_into(b, v, was_zero);
    mod_neg_inplace_fast(b, v, p);
    for i in 0..v.len() {
        if bit(p, i) {
            b.cx(was_zero, v[i]);
        }
...
pub fn build() -> Vec<Op> {
    ...
    // The epoch-fanout ledger is source-position-bound to the former Kaliski
    // caller.  The binary-GCD caller has a different frozen stream, so applying
    // those witnesses would be unsound; return the newly emitted stream as-is.
    b.ops.clone()
}
