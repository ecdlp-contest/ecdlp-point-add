...
pub fn build() -> Vec<Op> {
    // Research-only opt-in for validating the paper-signature replay with the
    // trusted contest pipeline. The default contest implementation is unchanged.
    if std::env::var_os("QARTON_FIXED_PORT").is_some() {
        return qarton_fixed_port::build_fixed_point_port();
    }
    // The four-register, reliability-hardened Qarton adaptation is the active
    // candidate. Set LEGACY_POINT_ADD only for local comparison with the parent.
    if std::env::var_os("LEGACY_POINT_ADD").is_none() {
        return qarton_classical_q_port::build_classical_q_port();
    }
    let b = &mut B::new();
